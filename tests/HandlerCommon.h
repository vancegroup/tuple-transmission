...
typedef transmission::Receiver<TestHandler, MyMessageCollection> TestReceiver;
...
BOOST_AUTO_TEST_CASE(DefaultConstruction) {
	...
	TestReceiver r;
	BOOST_CHECK_EQUAL(r.getMessageHandler().first, 0);
	BOOST_CHECK_EQUAL(r.getMessageHandler().second, 0);
	BOOST_CHECK_EQUAL(r.getMessageHandler().third, 0);
}
...
BOOST_AUTO_TEST_CASE(CompleteMessage) {
	...
	BOOST_CHECK_EQUAL(r.getMessageHandler().first, 5);
	BOOST_CHECK_EQUAL(r.getMessageHandler().second, 10);
	BOOST_CHECK_EQUAL(r.getMessageHandler().third, 15);
}
